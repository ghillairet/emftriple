...
public class SesameNativeResourceImpl 
	extends SailResourceImpl{

	public SesameNativeResourceImpl(URI uri) {
		super(uri);
	}
	...
	public IDataSource<Graph, Statement, Value, org.openrdf.model.URI, Literal> getDataSource(Map<?, ?> options) {
		if (options != null) {
			if (options.containsKey(ETripleOptions.OPTION_DATASOURCE_LOCATION)) {
				...
				return new SesameNative(new SailRepository(new NativeStore(file, indexes)));
				...
				return new SesameNative(repository);
			} else {
				return new SesameNative(new SailRepository(new NativeStore()));
			}
		}
		return null;
	}
